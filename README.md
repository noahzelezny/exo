...
| `EXO_MTP` | Enable MTP (multi-token prediction) speculative decoding at launch on models that ship an `mtp-head-q6.safetensors` sidecar (Qwen3.8-Flash, GLM-5.3-Flash, Qwen3.5-397B rungs from TheDrainFlorist). Easiest spelling: launch each node with `exo --mtp` (same mechanism). This launch rule selects the sequential engine; the engine-mode file below supersedes it at task boundaries, and on a single-node instance the batch engine drafts too (see `EXO_MTP_DRAFT_MIN_ACCEPT`). Unset = the launch rule leaves drafting inert. | unset |
| `EXO_ENGINE_MODE_FILE` | Path of the per-node engine-mode file (default `~/.exo/engine-mode`). Holding `sequential`, `batch` or `auto`, it switches a single-node instance between the sequential engine (one request at a time, MTP drafting when the model ships a head) and the batch engine (concurrent serving; on a single-node instance it drafts as well) at the next task boundary — no restart, no reload; weights and the prefix cache carry over. `auto` stays on the drafting batch engine where one exists; on a sharded instance it batches when another request is waiting and drafts sequentially otherwise. Absent or unrecognised = the launch-time rule above. | unset |
| `EXO_MTP_DRAFT_MIN_ACCEPT` | Batched drafting's floor: the batch engine drafts while (running acceptance)**rows stays at or above it, and takes plain one-token steps above that width, because a rejection in ANY row costs the whole batch a replay forward. 0.5 is the measured crossover for a ~0.75-acceptance head (drafting won at 2 rows, lost at 3); a ~0.9 head drafts to ~6 rows at the same floor. | `0.5` |
| `EXO_MTP_BATCH_MAX_ROWS` | Force a fixed row ceiling for batched drafting instead of the adaptive floor above. | unset |
