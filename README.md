...
| `EXO_MTP_BATCH_MAX_ROWS` | Force a fixed row ceiling for batched drafting. Unset, the batch engine TIMES both regimes per row count — speculative 2-token steps and plain 1-token steps — and takes the cheaper per committed token, re-trying the loser every ~96 steps; a rejection in ANY row costs the whole batch a replay forward, and how much a 2-wide forward costs over a 1-wide one differs per model, quant and box, so only a measurement decides. | unset |
