...
Observed live on Scout-17B (2026-07-15): with no parser the markup streamed
to Scout's chat as raw text, and without <|eom|> in the stop set the model
ran away re-emitting the same call as fresh assistant turns.
"""
...
def test_parses_wrapped_call():
    ...
    assert items[0].name == "recall_transcript"
    assert json.loads(items[0].arguments) == {"session": "last"}
...
def test_parses_bare_json_between_markers_stripped_by_stream():
    ...
    assert items is not None and items[0].name == "recall_transcript"
...
def test_garbage_returns_none_not_raise():
    ...
    assert parser.parse('<|python_start|>{"no_name": 1}<|python_end|>', tools=None) is None
...
def test_llama4_eos_includes_eom_and_eot():
    ids = get_eos_token_ids_for_model("mlx-community/Llama-4-Scout-17B-16E-Instruct-8bit")
...
def _stream(texts: list[str]) -> _Gen[GenerationResponse]:
...
def test_stream_wrapped_call_yields_tool_calls():
    ...
    assert len(tcs) == 1 and tcs[0].tool_calls[0].name == "recall_transcript"


def test_stream_bare_json_call_yields_tool_calls():
    # what Scout's tensor instance emitted live: no markers, just JSON + eos
    parser = make_llama4_parser()
    chunks = ['{"', "name", '": "recall_transcript", "parameters":', ' {"session": "last"}}']
    out = list(parse_tool_calls(_stream(chunks), parser, _TOOLS))
    tcs = [r for r in out if isinstance(r, ToolCallResponse)]
    assert len(tcs) == 1
    assert tcs[0].tool_calls[0].name == "recall_transcript"
    assert json.loads(tcs[0].tool_calls[0].arguments) == {"session": "last"}
